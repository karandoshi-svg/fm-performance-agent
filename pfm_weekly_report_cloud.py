...
# ── US + EU metrics: use pre-fetched data if available, else query Superset ───
...
if prefetched and prefetched.get('rpt_start') == rpt_start:
    ...
    print("Fetching US weekly metrics... (from pre-fetched data)")
    uw = prefetched['us_week'];  up = prefetched['us_prev'];  uq = prefetched['us_qtd']
    us_w_spend  = uw.get('spend', 0) or 0
    us_w_nfas   = int(uw.get('nfas', 0) or 0)
    us_w_roi    = uw.get('roi', 0) or 0
    us_p_spend  = up.get('spend', 0) or 0
    us_p_nfas   = int(up.get('nfas', 0) or 0)
    us_p_roi    = up.get('roi', 0) or 0
    us_qtd_sp   = uq.get('spend', 0) or 0
    us_qtd_nfas = int(uq.get('nfas', 0) or 0)
    us_qtd_gold = int(uq.get('gold', 0) or 0)
    print("Fetching EU metrics... (from pre-fetched data)")
    eu_w_spend  = prefetched.get('eu_w_spend', 0)
    eu_p_spend  = prefetched.get('eu_p_spend', 0)
    eu_w_nfas   = int(prefetched.get('eu_w_nfas', 0))
    eu_p_nfas   = int(prefetched.get('eu_p_nfas', 0))
    eu_w_roi    = prefetched.get('eu_w_roi', 0)
    eu_p_roi    = prefetched.get('eu_p_roi', 0)
else:
    # Fallback: query Superset directly (only works on Robinhood's network)
    print("Fetching US weekly metrics... (querying Superset directly)")
    ...
    us_qtd  = mcp_sql(f"""SELECT SUM(cost_amount) as spend, SUM(nfa_amount) as nfas, SUM(gold_amount) as gold
        FROM pfm_2025_report WHERE dt_epoch_ms >= '{qtd_start}' AND dt_epoch_ms < '{qtd_end_s}'
    ...
    print("Fetching EU metrics... (querying Superset directly)")
    ...
    eu_w_spend = row_total(eu_sp_curr);  eu_p_spend = row_total(eu_sp_prev)
    eu_w_nfas  = int(row_total(eu_nf_curr));  eu_p_nfas = int(row_total(eu_nf_prev))
